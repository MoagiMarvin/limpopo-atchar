...
const fallbackProducts = [
  { name: "Mango Atchar", category: "Mango Atchar", size: "1kg", price: 80, badge: "Best Seller", stock: 20, image: "/mango-atchar.png", active: true },
  { name: "Mango Atchar", category: "Mango Atchar", size: "2kg", price: 150, badge: "New", stock: 20, image: "/mango-atchar.png", active: true },
  { name: "Mango Atchar", category: "Mango Atchar", size: "5kg", price: 300, badge: "", stock: 20, image: "/mango-atchar.png", active: true },
  { name: "Mango Atchar", category: "Mango Atchar", size: "10kg", price: 550, badge: "", stock: 20, image: "/mango-atchar.png", active: true },
  { name: "Garlic Atchar", category: "Garlic Atchar", size: "1kg", price: 85, badge: "", stock: 20, image: "/mango-atchar-fallback.png", active: true },
  { name: "Garlic Atchar", category: "Garlic Atchar", size: "2kg", price: 160, badge: "", stock: 20, image: "/mango-atchar-fallback.png", active: true },
  { name: "Garlic Atchar", category: "Garlic Atchar", size: "5kg", price: 320, badge: "", stock: 20, image: "/mango-atchar-fallback.png", active: true },
  { name: "Garlic Atchar", category: "Garlic Atchar", size: "10kg", price: 580, badge: "", stock: 20, image: "/mango-atchar-fallback.png", active: true },
  { name: "Hot Chili Atchar", category: "Hot Chili Atchar", size: "1kg", price: 85, badge: "", stock: 20, image: "/mango-atchar-fallback.png", active: true },
  { name: "Hot Chili Atchar", category: "Hot Chili Atchar", size: "2kg", price: 160, badge: "", stock: 20, image: "/mango-atchar-fallback.png", active: true },
  { name: "Hot Chili Atchar", category: "Hot Chili Atchar", size: "5kg", price: 320, badge: "", stock: 20, image: "/mango-atchar-fallback.png", active: true },
  { name: "Hot Chili Atchar", category: "Hot Chili Atchar", size: "10kg", price: 580, badge: "", stock: 20, image: "/mango-atchar-fallback.png", active: true },
];
...
export default function Storefront() {
  ...
  const visibleProducts = useMemo(() => products.filter((product) => product.active !== false && (selectedCategory === "All" || (product.category || product.name) === selectedCategory)), [products, selectedCategory]);
  const total = cart.reduce((sum, item) => sum + Number(item.price), 0);

  function downloadReceipt(order) {
    const receipt = ["LIMPOPO ATCHAR RECEIPT", `Order: ${order.order_number}`, `Confirmation code: ${order.confirmation_code}`, "", `Customer: ${order.customer_name}`, `Phone: ${order.phone}`, `Delivery: ${order.address}, ${order.city}`, "Payment: Cash on Delivery", ...order.items.map((item) => `- Category: ${item.category || item.name} | ${item.name} (${item.size}) - R${item.price}`), `Delivery fee: R${order.delivery_fee}`, `Total: R${order.total}`].join("\n");
  ...
  async function submitOrder(event) {
    ...
    const receipt = ["LIMPOPO ATCHAR ORDER", `Confirmation code: ${code}`, "", `Customer: ${order.customer_name}`, `Phone: ${order.phone}`, `Delivery: ${order.address}, ${order.city}`, "Payment: Cash on Delivery", `Delivery fee: R${deliveryFee}`, "", "Items:", ...cart.map((item) => `- Category: ${item.category || item.name} | ${item.name} ${item.size} - R${item.price}`), "", `Total: R${order.total}`].join("\n");
  ...
  return <>
    ...
    <main id="home">
      ...
      <section className="section products-section" id="shop"><h2>Choose Your Atchar</h2><div className="categories" role="tablist" aria-label="Atchar categories"><button className={selectedCategory === "All" ? "active" : ""} onClick={() => setSelectedCategory("All")}><img src="/mango-atchar.png" alt="All Atchar" />All</button>{[{ name: "Mango Atchar", image: "/mango-atchar.png" }, { name: "Garlic Atchar", image: "/mango-atchar-fallback.png" }, { name: "Hot Chili Atchar", image: "/mango-atchar-fallback.png" }].map((category) => <button className={selectedCategory === category.name ? "active" : ""} key={category.name} onClick={() => setSelectedCategory(category.name)}><img src={category.image} alt={category.name} />{category.name}</button>)}</div><div className="ornament">— ❧ —</div><div className="products">{visibleProducts.map((product) => <article className="product" key={product.id || `${product.name}-${product.size}`}><span className="product-tag">{product.badge}</span><div className="product-photo"><img src={product.image || "/mango-atchar.png"} alt={`${product.name} ${product.size}`} /></div><div className="product-info"><h3>{product.name}</h3><p className="size-label">Container size: <b>{product.size}</b></p><div className="price">R{product.price}</div><button className="add" onClick={() => addToCart(product)} disabled={product.stock <= 0}>{product.stock <= 0 ? "Out of stock" : `🛒 Add ${product.size} to Cart`}</button></div></article>)}</div></section>
